...
const BookingPage = () => {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const safeCustomFields = Array.isArray(customFieldsRows) ? customFieldsRows :
                              (customFieldsRows && typeof customFieldsRows === 'object' && Array.isArray(customFieldsRows)) ? customFieldsRows :
                              [];
  ...
  const filtered = useMemo(() => {
    ...
    return data;
  }, [enrichedBookings, search, statusFilter, sortKey, sortDir]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!payload.customer_id || !payload.service_name || !payload.booking_date) {
  ...
  const handleCreateInvoice = async () => {
    ...
    if (existing) {
      alert(`Invoice already generated today (ID: ${existing.id}). Check Invoices module to view/print or update.`);
    ...
    try {
      ...
      alert("Invoice created and booking marked as completed. Check Invoices module to view/print.");
      setShowInvoiceModal(false);
    } catch (err) {
      console.error("Error generating invoice:", err);
      if ((err && err.message && err.message.includes("UNIQUE")) || (err && err.toString().includes("duplicate"))) {
              ...
              <tbody>
                {pageData.map((b) => {
                  const hasInvoiceToday = invoiceMap[b.customer_id]?.some(
                    (inv) => inv.total === (productMap[b.product_id]?.price || 0) && inv.invoice_date === new Date().toISOString().slice(0, 10)
                  );
                  return (
                    <tr key={b.id}>
                      <td>{b.id}</td>
                      ...
                      <td className="text-end">
                        <div className="d-flex justify-content-end gap-1 flex-wrap">
                          ...
                          <button
                            ...
                            title={hasInvoiceToday ? "Invoice already generated today" : ""}
                          >
                ...
                {!pageData.length && !loading && (
                  <tr>
                    <td colSpan={6 + (Array.isArray(customFields) ? customFields.filter(f => f.display_in_grid).length : 0)} className="text-center text-muted">
                      No bookings found.
                    </td>
                  </tr>
                )}
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  ...
                  <p>
                    ...
                    {(productMap[selectedBooking.product_id]?.price || 0).toFixed(2)}
                  </p>
